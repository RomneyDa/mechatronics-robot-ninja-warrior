...
void loop() 
{
  ...
  if (dist > thresholdDist && dist < 100){          // If the robot is to the right of the line
    Serial.print("Turn Left");                      // Turn left
    TurnLeft(MPow, driveTime);
  }
  else if (dist < -thresholdDist && dist > -100){   // If the robot is to the left of the line
    Serial.print("Turn Right");                     // Turn right
    TurnRight(MPow, driveTime);
  }
  else {                                            // Otherwise
    Serial.print("Drive Straight");                 // Go straight

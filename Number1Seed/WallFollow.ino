...
void WFState(){
  ...
  else if (Dfront < frontMax && t - tWF < 6.5){                                              // If the front distance sensor sees something, wall follow
    WallFollow();    
    MPow = 60;
  }
  
  else if (!noLine && t - tWF > 6.8){                      // Otherwise, if it sees a line, line follow (until side distance sees nothing!)
    LineFollow();
    MPow = 60;
  }
